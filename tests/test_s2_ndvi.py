...
@patch("remotepixel.s2_ndvi.sentinel2_get_info")
def test_point_valid(sentinel2_get_info, monkeypatch):
    ...
    sentinel2_get_info.return_value = {"cloud_coverage": 5.01, "sat": "S2B"}
...
@patch("remotepixel.s2_ndvi.sentinel2_get_info")
def test_point_invalid(sentinel2_get_info, monkeypatch):
    """Should work as expected and return 0 for outside point."""
    monkeypatch.setattr(s2_ndvi, "SENTINEL_BUCKET", sentinel_bucket)
    sentinel2_get_info.return_value = {"cloud_coverage": 5.01, "sat": "S2B"}
    expression = "(b08 - b04) / (b08 + b04)"
    coords = [-69.6140202938876, 38.25520824803732]
    expectedContent = {
        "date": "2017-07-29",
        "sat": "S2B",
        "scene": sentinel_scene,
...
@patch("remotepixel.s2_ndvi.sentinel2_get_info")
def test_area_valid(sentinel2_get_info, monkeypatch):
    """Should work as expected (read data, calculate NDVI and return img)."""
    monkeypatch.setattr(s2_ndvi, "SENTINEL_BUCKET", sentinel_bucket)
    sentinel2_get_info.return_value = {"cloud_coverage": 5.01, "sat": "S2B"}
    expression = "(b08 - b04) / (b08 + b04)"
    bbox = [-68.90625, 47.98992166741417, -67.5, 48.92249926375824]
    res = s2_ndvi.area(sentinel_scene, bbox, expression)
    assert res["date"] == "2017-07-29"
    assert res["sat"] == "S2B"
